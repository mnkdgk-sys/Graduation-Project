...
# モダンUIのカラーテーマ
COLORS = {
    'background': QColor(245, 245, 245),      # 明るいグレーの背景
    'surface': QColor(255, 255, 255),         # カードなどの表面 (白)
    'surface_light': QColor(238, 238, 238),   # 少し暗い表面 (ホバー時など)
    'primary': QColor(0, 123, 255),           # アクセントの青
    'primary_dark': QColor(0, 105, 217),      # アクセントの青 (暗め)
    'success': QColor(40, 167, 69),           # 緑 (成功)
    'warning': QColor(255, 193, 7),           # 黄 (警告)
    'danger': QColor(220, 53, 69),            # 赤 (危険)
    'text_primary': QColor(33, 37, 41),       # メインのテキスト (濃いグレー)
    'text_secondary': QColor(108, 117, 125),  # サブのテキスト (グレー)
    'text_muted': QColor(170, 170, 170),      # さらに薄いテキスト (グレー)
    'border': QColor(221, 221, 221),          # 境界線
    'note_glow': QColor(0, 123, 255, 100),    # ノートの光彩エフェクト
    'rest_bg': QColor(230, 230, 230, 200),    # 休符の背景
    'staff_line': QColor(50, 50, 50),         # 譜線 (濃いグレー)
    'cursor': QColor(255, 0, 100),            # 再生カーソル
}
...
def resource_path(relative_path):
    try:
        # PyInstallerで実行された場合、一時フォルダのパスを取得
        base_path = sys._MEIPASS
    except Exception:
        # 通常の実行の場合、このスクリプトファイルのディレクトリを取得
...
class RhythmWidget(QWidget):
    ...
    def draw_item(self, painter, item, staff_y, start_x, drawable_width, total_beats_on_track):
        ...
        if item.get('class') == 'note':
            ...
            # ▼▼▼ 変更箇所 ▼▼▼
            # 音符の背景を塗りつぶさず、枠線を描画するように変更
            painter.setBrush(Qt.BrushStyle.NoBrush) # 塗りつぶしを無効に
            pen_width = 2
            if item.get('is_lit', False):
                # 光っている場合は枠線を少し太くする
                pen_width = 4
            pen = QPen(COLORS['primary'], pen_width) # 太さを変数で指定
            painter.setPen(pen)
    ...
    def on_track_mode_changed(self, checked):
        ...
        if any(d.get('items') for d in self.rhythm_widget.score.values()):
            ...
            if reply == QMessageBox.StandardButton.No:
                ...
                self.bottom_track_settings.setVisible(not is_2_track); self.sync_checkbox.setVisible(not is_2_track)
                return
        self.new_score(ask_confirm=False)
...
def run_editor():
    ...
    # ウィンドウをマウスカーソルのあるスクリーンの中央に表示する処理
    try:
        screen = QApplication.screenAt(QCursor.pos())
        if not screen:
            # マウス位置からスクリーンが取得できない場合、プライマリスクリーンを使用
            screen = QApplication.primaryScreen()
        center_point = screen.availableGeometry().center()
        # ウィンドウの中心がスクリーンの中心に来るように左上の位置を計算して移動
        window.move(center_point.x() - window.width() / 2,
                    center_point.y() - window.height() / 2)
    except Exception as e:
        print(f"ウィンドウの中央配置中にエラーが発生しました: {e}")
        # 失敗した場合はデフォルト位置(100, 100)に表示
        window.move(100, 100)
