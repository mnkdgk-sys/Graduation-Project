...
def main():
    ...
    pygame.display.set_caption("リズムエディター Final (修正版)")
    clock = pygame.time.Clock()
    
    try: font = pygame.font.Font("NotoSansJP-Regular.ttf", 20); font_small = pygame.font.Font("NotoSansJP-Regular.ttf", 16)
    except FileNotFoundError: font = pygame.font.SysFont("sans-serif", 20); font_small = pygame.font.SysFont("sans-serif", 16)
    ...
    # 【UI位置修正】
    btn_bpm_minus = Button("-", (440, 40), font, padding=5)
    btn_bpm_plus = Button("+", (560, 40), font, padding=5)

    palette_buttons = {}
    x_offset, y_offset = 60, 120
    for i, (note_type, props) in enumerate(NOTE_DURATIONS.items()):
        button = Button(props['name'], (x_offset, y_offset + i * 50), font_small, padding=8); palette_buttons[note_type] = button
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 3: # 右クリック
                    if not is_playing:
                        mouse_pos = event.pos; note_to_delete = None
                        for note in score:
                            x = start_x + (note['beat'] / beats_per_measure) * measure_width; width = (note['duration'] / beats_per_measure) * measure_width
                            note_rect = pygame.Rect(x, staff_y - 25, width, 50)
                            if note_rect.collidepoint(mouse_pos): note_to_delete = note; break
                        if note_to_delete: score.remove(note_to_delete)
                
                elif event.button == 1: # 左クリック
                    if btn_start.is_clicked(event) and not is_playing: is_playing = True; playback_start_time = pygame.time.get_ticks()
                    elif btn_stop.is_clicked(event) and is_playing: is_playing = False
                    elif btn_reset.is_clicked(event) and not is_playing: score = []
                    elif btn_bpm_minus.is_clicked(event) and not is_playing: bpm = max(30, bpm - 5)
                    elif btn_bpm_plus.is_clicked(event) and not is_playing: bpm = min(240, bpm + 5)
                    else:
                        note_type_to_add = None
                        for note_type, button in palette_buttons.items():
                            if button.is_clicked(event): note_type_to_add = note_type; break
        ...
        bpm_text = font.render(f"BPM: {bpm:.0f}", True, C_BLACK); screen.blit(bpm_text, bpm_text.get_rect(center=(500, 40)))
