...
def draw_note(surface, note_type, pos, is_lit):
    note_head_radius = 10
    stem_height = 40
    color = C_RED if is_lit else C_BLACK # 光っているかどうかで色を変える
...
def main():
    pygame.init()
    pygame.mixer.init() # オーディオ機能の初期化
    screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
    pygame.display.set_caption("リズムエディター v2 - プレイヤー")
    font = pygame.font.SysFont("sans-serif", 30)
    font_small = pygame.font.SysFont("sans-serif", 20)

    # クリック音の読み込み (ファイルがなくてもエラーにならないように)
    try:
    ...
    score = [
        {'beat': 0, 'type': 'quarter'}, {'beat': 1, 'type': 'quarter'},
        {'beat': 2, 'type': 'quarter'}, {'beat': 3, 'type': 'quarter'},
    ]
    beats_per_measure = 4
    ...
    note_lit_times = [0] * len(score) # 各音符が光り始めた時刻を記録
    next_click_beat = 0 # 次に鳴らすべきクリック音の拍
    ...
    while True:
        ...
        bpm_text = font.render(f"BPM: {BPM}", True, C_BLACK)
        screen.blit(bpm_text, (260, 30))
        ...
        if is_playing:
            ...
            if current_beat_in_loop >= next_click_beat:
                if click_sound:
                    click_sound.play()
                # 次のクリックタイミングを更新
                next_click_beat = (next_click_beat + 1) % beats_per_measure
            
            # 音符ハイライト
            for i, note in enumerate(score):
                if current_beat_in_loop >= note['beat'] and (current_beat_in_loop - note['beat']) < 0.1: # わずかな間だけ判定
