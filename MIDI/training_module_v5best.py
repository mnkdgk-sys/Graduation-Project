...
class MainWindow(QMainWindow):
    ...
    def get_stats_per_pad(self):
        ...
        for pad in pads:
            ...
            weighted_sum = (stats['perfect'] * 1.0) + \
                           (stats['great']   * 0.7) + \
                           (stats['good']    * 0.4)
            stats['score'] = (weighted_sum / total_notes * 100) if total_notes > 0 else 0.0
            # ---------------------
    ...
    def evaluate_and_continue_loop(self):
        ...
        if elapsed_practice_time >= time_limit_seconds:
            ...
            if success and self.practice_loop_count_max == float('inf'): 
                ...
                if self.editor_window: self.editor_window.close()
            elif self.practice_loop_count >= self.practice_loop_count_max: 
                self.result_stats = stats
    ...
    def summarize_performance(self):
        ...
        # --- 2. Score (得点率): 重みづけスコア ★追加★ ---
        # Perfect=100%, Great=70%, Good=40% として計算
        # (好みで 0.8 や 0.5 に変更してください)
        weighted_sum = (stats['perfect'] * 1.0) + \
                       (stats['great']   * 0.7) + \
                       (stats['good']    * 0.4)
                       
        stats['score'] = (weighted_sum / self.total_notes * 100) if self.total_notes > 0 else 0
        # -----------------------------------------------
