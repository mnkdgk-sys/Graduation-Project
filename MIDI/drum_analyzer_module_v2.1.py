...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.is_recording_setup = False # ★修正: 録音準備が完了したかを示すフラグ
        self.judgements = []
    ...
    def _start_countdown(self):
        ...
        self.is_recording_setup = False # ★修正: 録音準備フラグをリセット
    ...
    def setup_recording(self):
        if self.is_recording_setup:
            return
        
        # 音楽の開始時刻（タイムラインのゼロ地点）を正確に定義
        self.start_time = self.countdown_start_time + (self.countdown_beat_duration * 4 / 1000.0)
    ...
    def judge_hit(self, hit):
        ...
        if closest_note and min_diff < clustering_threshold:
            ...
            # 絶対的な時間精度で perfect/great/good を判定
    ...
    def update_loop(self):
        if self.state == "practice_countdown":
            ...
            # 4拍目が始まったら（3拍目が終わったら）、録音準備を開始
            if elapsed_ms >= self.countdown_beat_duration * 3:
                self.setup_recording()

            # カウントダウンの表示更新と音の再生
            if current_beat != self.countdown_beat_count and current_beat < 4:
                self.play_countdown_sound()
                self.countdown_beat_count = current_beat
            ...
            elif elapsed_ms < self.countdown_beat_duration * 4:
                self.countdown_text = "START!"
            else:
                # カウントダウンが完全に終了したら練習ウィンドウを表示
                # 既に state は "recording" になっている
                self.countdown_text = "" 
