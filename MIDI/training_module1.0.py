...
class RobotController(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                for i, current_note in enumerate(self.note_items):
                    ...
                    if i + 1 < len(self.note_items):
                        ...
                        next_tap_time = next_loop_start_time + (self.note_items[0].get("beat", 0) * seconds_per_beat)
            ...
            self.log_message.emit(f"ロボット [{self.config.get('port', 'Unknown')}] エラー: {e}")
        finally:
            if device:
                try:
                ...
                self.log_message.emit(f"ロボット [{self.config.get('port', 'Unknown')}] 接続解除")
            self.finished.emit()
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🥁 ドラム&ロボット トレーナー")
        ...
        self.robot_threads = []
        self.robot_workers = []
        self.robot_start_event = threading.Event()
        self.robot_stop_event = threading.Event()
        self.robot_active_devices = []
    ...
    def start_generic_practice(self, is_perfect_mode):
        ...
        self.start_robot_control(master_loop_duration_ms)
    ...
    def start_robot_control(self, master_loop_duration_ms):
        self.stop_robot_control()
        self.robot_stop_event.clear()
        self.robot_start_event.clear()

        score_data = self.template_score
        top_score = score_data.get("top"); bottom_score = score_data.get("bottom")
        if not top_score or not bottom_score: return
        ...
        loop_duration_sec = master_loop_duration_ms / 1000.0
        configs = [(ROBOT1_CONFIG, top_notes, top_bpm), (ROBOT2_CONFIG, bottom_notes, bottom_bpm)]

        for config, notes, bpm in configs:
            if not notes: continue
            thread = QThread()
            worker = RobotController(config, notes, bpm, loop_duration_sec, self.robot_start_event, self.robot_stop_event, self.robot_active_devices)
            worker.moveToThread(thread)
            worker.log_message.connect(lambda msg: self.label_info.setText(msg))
            thread.started.connect(worker.run)
            worker.finished.connect(thread.quit)
            worker.finished.connect(worker.deleteLater)
            thread.finished.connect(thread.deleteLater)
            thread.finished.connect(lambda t=thread, w=worker: self.on_robot_thread_finished(t, w))
            thread.start()
            self.robot_threads.append(thread)
            self.robot_workers.append(worker)
...
class EditorWindow(QMainWindow):
    ...
    def start_actual_playback(self):
        ...
        self.main_window.robot_start_event.set()
