...
from PyQt6.QtGui import (
    QPainter, QColor, QFont, QIcon, QPen, QPixmap, QLinearGradient, QCursor, QFontDatabase, QPolygonF
)
...
# --- 設定定数 ---
KEY_MAPPING = {'f': 'top', 'j': 'bottom'} # 'f'キー -> 左手(top), 'j'キー -> 右手(bottom)
...
def load_controllers():
    """'controllers' ディレクトリから BaseEntrainmentController のサブクラスを動的に読み込む"""
    controllers = {}
    
    # ★修正点1: スクリプト自身の場所を基準にする
    ...
    # ★修正点2: スクリプトの親ディレクトリをパスに追加する
    sys.path.insert(0, os.path.dirname(script_dir))
    
    try:
        from controllers.base_controller import BaseEntrainmentController
        for filename in os.listdir(controller_dir):
            if filename.endswith(".py") and filename != "base_controller.py" and filename != "__init__.py":
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if issubclass(obj, BaseEntrainmentController) and obj is not BaseEntrainmentController:
                            try:
                               ...
                               print(f"Error instantiating controller {name} for name check: {e}")
                except ImportError as e:
                    print(f"Failed to import controller module {module_name}: {e}")
    except ImportError as e:
        print(f"Could not import BaseEntrainmentController. Ensure 'controllers/__init__.py' exists. Error: {e}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ドラムリズムアナライザー (シミュレーションモード)")
    ...
    def init_ui(self):
        ...
        # コントローラー選択のドロップダウンを作成
        self.label_controller = ModernLabel("制御方法:", 9)
    ...
    def on_controller_changed(self):
        """ドロップダウンで選択されたコントローラーをインスタンス化する"""
        ...
        if selected_class:
            ...
            self.active_controller = selected_class(self.template_score, ms_per_beat)
    ...
    def init_keyboard_listener(self):
        try:
            ...
            self.label_info.setText("✅ シミュレーションモード: 'F'キー (左手), 'J'キー (右手) で入力してください。")
        except Exception as e:
            self.label_info.setText(f"❌ キーボード監視エラー: {e}")
    ...
    def on_key_press(self, key):
        ...
        try:
            ...
            if char in KEY_MAPPING:
                pad = KEY_MAPPING[char]
                self.process_key_hit(pad)
        except AttributeError:
            pass

    def process_key_hit(self, pad):
    ...
    def start_demo_playback(self):
        ...
        # デモ再生時は介入を行わないコントローラーに設定し、シミュレーターを生成
        if self.robot_simulator: self.robot_simulator.close()
        passthrough_index = self.control_combo.findText("介入なし (お手本通り)")
        if passthrough_index != -1:
            self.control_combo.setCurrentIndex(passthrough_index)
        
        self.on_controller_changed() # 選択を反映してコントローラーをインスタンス化
        self.robot_simulator = RobotSimulatorWindow(self.editor_window, self.active_controller)
        self.robot_simulator.show()
    ...
    def start_actual_recording(self):
        ...
        # 練習開始前にコントローラーを最新の状態に更新してから、シミュレーターを生成
        self.on_controller_changed()
        if self.robot_simulator: self.robot_simulator.close()
        self.robot_simulator = RobotSimulatorWindow(self.editor_window, self.active_controller)
        self.robot_simulator.show()
    ...
    def evaluate_and_continue_loop(self):
        ...
        if self.robot_simulator:
            self.robot_simulator.update_controller_performance_data(self.judgement_history)
...
# --- ロボットシミュレーター関連クラス ---
# -----------------------------------------------------------------------------
class RobotCircleWidget(QWidget):
    """ロボット1台の動作（光の点灯）を表現する円形ウィジェット"""
    ...
    def set_lit(self, lit_state):
        if self.is_lit != lit_state:
            self.is_lit = lit_state
            self.update()
    ...
    def paintEvent(self, event):
        ...
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        painter.setPen(COLORS['text_primary'])
        painter.setFont(QFont("SansSerif", 12, QFont.Weight.Bold))
        painter.drawText(self.rect(), Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignTop, self.label_text)
        center = self.rect().center()
        radius = 60
        if self.is_lit:
            gradient = QLinearGradient(center.x(), center.y() - radius, center.x(), center.y() + radius)
            gradient.setColorAt(0, self.base_color.lighter(130)); gradient.setColorAt(1, self.base_color)
            glow_color = QColor(self.base_color); glow_color.setAlpha(80)
            painter.setPen(Qt.PenStyle.NoPen); painter.setBrush(glow_color)
            painter.drawEllipse(center, radius, radius)
            painter.setBrush(gradient); painter.setPen(QPen(self.base_color.darker(110), 2))
            painter.drawEllipse(center, radius - 5, radius - 5)
        else:
            color = self.base_color.darker(150); color.setAlpha(100)
            painter.setBrush(COLORS['surface']); painter.setPen(QPen(color, 2, Qt.PenStyle.DotLine))
            painter.drawEllipse(center, radius - 5, radius - 5)


class RobotSimulatorWindow(QWidget):
    """
    2台のロボットの動作をシミュレーションするウィンドウ（表示機能のみ）
    """
    def __init__(self, editor_window, active_controller, parent=None):
        super().__init__(parent)
        self.editor_window = editor_window
        self.score = self.editor_window.main_window.template_score
        
        self.active_controller = active_controller
        
        self.setWindowTitle("🤖 ロボット動作シミュレーター")
        self.setFixedSize(400, 250)
        self.setStyleSheet(f"background-color: {COLORS['background'].name()};")

        self.left_robot = RobotCircleWidget("ロボット1 (左手)", COLORS['primary'])
        self.right_robot = RobotCircleWidget("ロボット2 (右手)", COLORS['success'])

        layout = QHBoxLayout(self)
        layout.addWidget(self.left_robot)
        layout.addWidget(self.right_robot)
        
        self.simulation_timer = QTimer(self)
        self.simulation_timer.timeout.connect(self.update_simulation)
        self.simulation_timer.start(16)

    def update_controller_performance_data(self, full_judgement_history):
        if self.active_controller:
            self.active_controller.update_performance_data(full_judgement_history)

    def update_simulation(self):
        if not self.editor_window or not self.editor_window.isVisible():
            return
            
        elapsed_ms = self.editor_window.get_elapsed_time()
        
        left_lit = self.check_track_lit_state('top', elapsed_ms)
        self.left_robot.set_lit(left_lit)
        right_lit = self.check_track_lit_state('bottom', elapsed_ms)
        self.right_robot.set_lit(right_lit)

    def check_track_lit_state(self, track_name, elapsed_ms):
        if track_name not in self.score or self.active_controller is None:
            return False
            
        loop_duration_ms = self.editor_window.rhythm_widget.get_loop_duration()
        if loop_duration_ms <= 0:
            return False
        time_in_loop = elapsed_ms % loop_duration_ms

        ms_per_beat = self.active_controller.ms_per_beat

        for item in self.score[track_name].get('items', []):
            if item['class'] == 'note':
                ideal_time = item['beat'] * ms_per_beat
                guided_time = self.active_controller.get_guided_timing(track_name, ideal_time)
                
                if guided_time <= time_in_loop < guided_time + LIT_DURATION:
                    return True
        return False
